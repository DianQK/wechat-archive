...
RUN apt-get update && apt-get install -y sqlcipher rsync && rm -rf /var/lib/apt/lists/*
...
CMD ["wechat-archive", "--config","/wechat-archive.toml", "--port", "8080", "--static-dir", "/dist", "--assets-dir", "/assets"]
EXPOSE 8080
